...
def calculate_shipping(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Remaining1"] = np.floor(df["Total Weight"] - shipping_costs1["Minimum KG"])
    ...
    df["Remaining2"] = np.floor(df["Total Weight"] - shipping_costs2["Minimum KG"])
    ...
    df["Remaining3"] = np.floor(df["Total Weight"] - shipping_costs3["Minimum KG"])
