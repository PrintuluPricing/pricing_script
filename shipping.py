...
def calculate_shipping(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Shipping Costs1"] = df["Shipping Costs1"] * (1 + shipping_costs1["fuel_surcharge"])
    ...
    df["Shipping Costs2"] = df["Shipping Costs2"] * (1 + shipping_costs2["fuel_surcharge"])
    ...
    df["Shipping Costs3"] = df["Shipping Costs3"] * (1 + shipping_costs3["fuel_surcharge"])
